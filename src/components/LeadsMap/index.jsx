...
export const LeadsMap = ({
  ...
  const [userClosedStatsPanel, setUserClosedStatsPanel] = useState(false);
  ...
  const handleDMAClick = useCallback((dmaId, dmaName) => {
          ...
          setTimeout(() => {
            ...
            if (mapRef.current) {
              ...
              if (bounds) {
                ...
                const viewportInfo = {
                  center: [currentViewport.lng, currentViewport.lat],
                  bounds: {
  ...
  const handleDMAHover = useCallback((dmaId) => {
    setHoveredDMA(dmaId);
  }, []);
  ...
  const handleCountyHover = useCallback((county) => {
    if (county) {
  ...
  const handleViewportChange = useCallback((newViewport) => {
    ...
    if (!isZoomedToDMA && newViewport.zoom >= 8) {
      ...
      const mapCenter = [newViewport.longitude, newViewport.latitude];
  ...
  useEffect(() => {
    ...
    if (hasSelectedCounties && !userClosedStatsPanel && !showCountyStatsPanel) {
      setShowCountyStatsPanel(true);
    }


    if (!hasSelectedCounties) {
      setUserClosedStatsPanel(false);
    }
  }, [selectedCounties.length, showCountyStatsPanel, userClosedStatsPanel]);
  ...
  const handleMapClick = useCallback(async (event) => {
    ...
    if (event.features && event.features.length > 0) {
      ...
      if (isZoomedToDMA && showCountiesInDMA) {
        const countyFeature = event.features.find((f) =>
          f.layer && (
      ...
      const dmaFeature = event.features.find((f) =>
  ...
  const getMarkerSize = (totalCalls) => {
            ...
            <div className="flex items-center justify-between sm:justify-start space-x-2 sm:space-x-4">
              <div className="flex items-center space-x-2">
                ...
                {selectedCounties.length > 0 && (
                  <button
                    onClick={() => {
                      setShowCountyStatsPanel(!showCountyStatsPanel);
                      if (showCountyStatsPanel) {
                        setUserClosedStatsPanel(true);
                      } else {
                        setUserClosedStatsPanel(false);
                      }
                    }}
